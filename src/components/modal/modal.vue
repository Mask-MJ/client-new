...
import { useIsMobile, usePriorityValues, useSimpleLocale } from '@/components/layouts/hooks';
import { ELEMENT_ID_MAIN_CONTENT } from '@/config/constants';
import { globalShareState } from '@/utils';
// import {
//   DialogContent,
//   DialogDescription,
//   DialogFooter,
...
import { CircleHelp, Expand, Shrink } from 'lucide-vue-next';
...
<template>
  ...
  >
    <DialogContent
      ...
      class="left-0 right-0 top-[10vh] mx-auto flex max-h-[80%] w-[520px] flex-col p-0 sm:rounded-[var(--radius)]"
      :class="[
        modalClass,
        {
    ...
    >
        ...
        <DialogTitle v-if="title" class="text-left">
          <slot name="title">
            ...
            <slot v-if="titleTooltip" name="titleTooltip">
              <n-tooltip trigger="hover" class="pb-1" side="right">
                <template #trigger>
                  <CircleHelp />
                </template>
                {{ titleTooltip }}
      ...
      <div
        ref="wrapperRef"
        class="relative min-h-40 flex-1 overflow-y-auto p-3"
        :class="[
          contentClass,
          {
      ...
      >
        <Loading v-if="showLoading || submitting" class="size-full h-auto min-h-full" spinning />
        <slot></slot>
      </div>
      ...
      <DialogFooter
        v-if="showFooter"
        ref="footerRef"
        class="flex-row items-center justify-end p-2"
        :class="[
          {
            'border-t': bordered,
          },
      ...
      >
        ...
        <slot name="footer">
          <component
            :is="components.DefaultButton || NButton"
          ...
          <component
            :is="components.PrimaryButton || NButton"
